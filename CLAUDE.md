...
- **Categories** = the 10 topics The Trivia API backs, defined once in `src/lib/categories.ts` (the single source of truth). Each entry has a numeric `openTdbId` (the app's internal id, used by `/trivia/[category]` SEO pages and `/play?category=<id>` links) and a `triviaApiCategory` slug. `src/lib/triviaApi.ts` derives both the id→slug map (`CATEGORY_ID_TO_API_SLUG`) and the picker list (`PLAYABLE_CATEGORIES`) from that array — add/rename a category in one place. The app was consolidated from 24 narrower categories down to these 10; the retired slugs (video-games, anime-manga, etc.) **301-redirect** to their surviving parent via `CATEGORY_REDIRECTS` in `next.config.ts`. `fetchCategories()` returns the list with no network call.
